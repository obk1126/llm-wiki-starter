...
`install.sh`는 `skills/` 하위 스킬 전부를 `~/.claude/skills/`에 복사한다. 그게 전부다.

## 들어 있는 스킬 3종

| 스킬 | 역할 |
|------|------|
| `llm-wiki` | 볼트 셋업·인제스트·쿼리·린트 — 세컨드 브레인의 본체 |
| `wiki-dispatch` | **방법론 라우터** — 작업 유형별 필독 노트를 실행 전에 로딩. "스킬은 껍데기, 지식은 위키" 원칙과 노트→스킬 승격 3질문 포함 |
| `verification-panel` | **페르소나 검증단** — 대외 산출물을 5석(구매자·리스크·실사용자·양식감사·사실검증) 병렬 에이전트로 제출 전 상호검증 |

위키가 자라면 `wiki-dispatch`의 라우팅 표에 방법론 노트를 한 줄씩 등록하라 — 지식은 노트에서 자라고, 스킬은 경로만 안다. 이것이 위키(지식)와 스킬(실행)을 잇는 최소 구조다.
